...
features are computed strictly from information available at the cutoff:
  ...
  * 8 x 15-min demand lags, trailing rolling means, prior-turn demand
  * neighbouring-zone demand (from the k-nearest game-zone graph)
...
Output: ``train_dataset.parquet`` split into train / validation / holdout.
"""
...
def build() -> pd.DataFrame:
    ...
    feature_cols = config.FEATURE_ORDER + ["target", "cutoff", "game_zone", "split"]
    ds = ds[feature_cols]
    ds = ds.dropna()
...
def main() -> None:
    ds = build()
    out = config.ML_ARTIFACTS / "train_dataset.parquet"
